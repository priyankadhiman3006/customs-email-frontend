  ...
  "SignOut Controller" should {
    "redirect to feedback survey" in new Setup {
      running(app) {
        ...
        redirectLocation(result).get should endWith("/feedback/manage-email-cds")
      }
    }
    ...
    "clear the session once the user signs out" in new Setup {
      running(app) {
        ...
        val result = route(app, signOutRequest).value
        status(result) shouldBe SEE_OTHER
        redirectLocation(result).get should endWith("/feedback/manage-email-cds")

        val startPageRequest = FakeRequest(GET, routes.ApplicationController.start().url)

        val resultStartPage = route(app, startPageRequest).value
        status(resultStartPage) shouldBe OK
