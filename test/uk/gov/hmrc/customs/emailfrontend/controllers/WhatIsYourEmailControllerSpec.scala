...
class WhatIsYourEmailControllerSpec extends SpecBase with BeforeAndAfterEach {
  ...
  private val mockSubscriptionDisplayConnector: SubscriptionDisplayConnector = mock[SubscriptionDisplayConnector]
  ...
  "WhatIsYourEmailController" should {
    ...
    "have a status of OK for create method when verified email found in subscription display response" in new Setup  {
      ...
      when(mockEmailVerificationService.isEmailVerified(meq(someSubscriptionDisplayResponse.email.get))(any[HeaderCarrier])
      ).thenReturn(Future.successful(Some(true)))
    ...
    "show 'ineligible user' page for an authorised user without eori accessing create" in new Setup {
      //TODO:- withAuthorisedUserWithoutEori
      //already tested in identifier action spec

      running(app) {

        val request = FakeRequest(POST, routes.WhatIsYourEmailController.create().url)
